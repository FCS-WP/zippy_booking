...
const BookingStep1 = ({ handleNextStep }) => {
  ...
  const getCategories = async () => {
    ...
    response.data.categories[0].subcategories.length > 0 ? handleLoadProducts(response.data.categories[0].subcategories[0]) : handleLoadProducts(response.data.categories[0]);
  }

  const handleLoadProducts = (category) => {
    if (category.subcategory_id) {
      setProducts([...category.subcategory_products]);
    } else {
      setProducts([...category.products_in_category]);
    }
  }
                ...
                if (category.subcategories.length > 0) {
                  return category.subcategories.map((subCategory, subIndex)=>(
                    <div
                      ...
                      className={`category-item ${
                        activeCategory == (x1000(index)+subIndex) ? "active" : ""
                      } `}
                      role="button"
                      onClick={() => handleSelectCategory(subCategory, (x1000(index)+subIndex))}
                    >
                  ...
                  return (
                    <div
                      ...
                      role="button"
                      onClick={() => handleSelectCategory(category, x1000(index))}
                    >
